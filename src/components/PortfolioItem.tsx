        ...
        <div className="w-full h-48 p-4">
          ...
          <p className="flex flex-wrap gap-2 flex-row items-center justify-start text-xs md:text-sm">
            {stack && stack.length > 0 && stack.map((item, index) => {
              return (
                <div>
                  <span className="transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 hover:bg-indigo-500 duration-300 inline-block text-xs px-1 py-1 mb-1 font-semibold border-2 border-stone-900 rounded-md" key={index}> {item} </span>
                </div>
              );
            })}
          </p>
