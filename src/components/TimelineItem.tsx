...
const TimelineItem: React.FC<TimelineItemProps> = ({ year, title, duration, details }) => {
    ...
    return (
        <ol
            ref={ref}
            className={`max-w-screen-sm flex hover:bg-slate-100 rounded-md flex-col md:flex-row relative border-l border-stone-200 dark:border-stone-700 transition-all duration-500 transform ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}
        >
            <li className="max-w-screen-sm mb-10 ml-4">
